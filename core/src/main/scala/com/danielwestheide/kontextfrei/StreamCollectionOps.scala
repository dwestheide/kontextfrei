...
trait StreamCollectionOps {
  implicit val streamCollectionOps: DCollectionOps[Stream] =
    new DCollectionOps[Stream] {
      def unit[A: ClassTag](as: Seq[A]): Stream[A] = as.toStream
      def cartesian[A: ClassTag, B: ClassTag](as: Stream[A])(
      ...
      def cogroup[A: ClassTag, B: ClassTag, C: ClassTag](x: Stream[(A, B)])(
      ...
      def values[A: ClassTag, B: ClassTag](x: Stream[(A, B)]): Stream[B] =
        x.map(_._2)
      def keys[A: ClassTag, B: ClassTag](x: Stream[(A, B)]): Stream[A] =
        x.map(_._1)

      def leftOuterJoin[A: ClassTag, B: ClassTag, C: ClassTag](x: Stream[
                                                                 (A, B)])(
          y: Stream[(A, C)]): Stream[(A, (B, Option[C]))] = {
      ...
      def rightOuterJoin[A: ClassTag, B: ClassTag, C: ClassTag](x: Stream[
                                                                  (A, B)])(
          y: Stream[(A, C)]): Stream[(A, (Option[B], C))] = {
        flatMapValues(cogroup(x)(y)) {
          case (bs, cs) =>
      ...
      def reduceByKey[A: ClassTag, B: ClassTag](xs: Stream[(A, B)])(
          f: (B, B) => B): Stream[(A, B)] = {
