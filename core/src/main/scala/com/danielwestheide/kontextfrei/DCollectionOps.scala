...
trait DCollectionOps[DCollection[_]] {
  ...
  // transformations:
  def cartesian[A : ClassTag, B : ClassTag](as: DCollection[A])(bs: DCollection[B]): DCollection[(A, B)]
  def collect[A : ClassTag, B : ClassTag](as: DCollection[A])(pf: PartialFunction[A, B]): DCollection[B]
  def distinct[A : ClassTag](as: DCollection[A]): DCollection[A]
  def map[A : ClassTag, B : ClassTag](as: DCollection[A])(f: A => B): DCollection[B]
...
object DCollectionOps {
  ...
  trait DistributionOps {
    def unit[DCollection[_], A: ClassTag]
    (as: Seq[A])
    (implicit dc: DCollectionOps[DCollection]): DCollection[A] = dc.unit(as)
  }

  class Syntax[DCollection[_], A : ClassTag](val self: DCollectionOps[DCollection], val coll: DCollection[A]) {
    def cartesian[B : ClassTag](bs: DCollection[B]): DCollection[(A, B)] = self.cartesian(coll)(bs)
    def collect[B : ClassTag](pf: PartialFunction[A, B]): DCollection[B] = self.collect(coll)(pf)
    ...
    def countByValue()(implicit ord: Ordering[A]): Map[A, Long] = self.countByValue(coll)
  ...
  trait ToSyntax {
    ...
    (implicit dc: DCollectionOps[DCollection]) = new Syntax[DCollection, A](dc, target)
    implicit def toDCollectionPairOps[DCollection[_], A : ClassTag, B : ClassTag]
    (target: DCollection[(A, B)])
    (implicit dc: DCollectionOps[DCollection]) = new PairSyntax[DCollection, A, B](dc, target)
  }
